...
{/* Header Section */}
<div className="flex flex-col mb-8 shadow:md">
    ...
    <table className="min-w-full border border-gray-200">
      <thead className="bg-gray-50">
        <tr>
          ...
          <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-r border-gray-200">Avatar</th>
          <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-r border-gray-200">Roll No</th>
          ...
          <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-r border-gray-200">Department</th>
          <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-r border-gray-200">Mobile</th>
          ...
          <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-r border-gray-200">Admission Date</th>
      ...
      <tbody className="bg-white divide-y divide-gray-200">
        {displayedStudents.map((student, index) => (
          ...
          >
            <td className="flex justify-center mb-1 mt-1">
  <Image 
    src={`https://i.pravatar.cc/40?img=${student.id}`} 
    alt={student.name} 
    width={32}   // bigger avatar
    height={32}
    className="rounded-full mr-2" 
  />
</td>
<td className="px-6 py-2 whitespace-nowrap text-sm text-gray-700 border-r border-gray-200">{student.rollNo}</td>
<td className="px-6 py-2 whitespace-nowrap text-sm font-medium text-gray-900 border-r border-gray-200">
  <div className="flex items-center">{student.name}</div>
</td>
<td className="px-6 py-2 whitespace-nowrap text-sm text-gray-700 border-r border-gray-200">{student.department}</td>
<td className="px-6 py-2 whitespace-nowrap text-sm text-gray-700 border-r border-gray-200">
  <a 
...
<td className="px-6 py-2 whitespace-nowrap text-sm font-medium flex space-x-2">
  <button className="p-2 rounded-full bg-green-200 hover:bg-green-300">
    <EditIcon className="w-5 h-5 text-green-700" />
  </button>
  <button className="p-2 rounded-full bg-red-200 hover:bg-red-300">
    <DeleteIcon className="w-5 h-5 text-red-700" />
  </button>
</td>
...
{/* Grid View - 3 Columns with Fixed Rows */}
{viewMode === 'grid' && (
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
    {/* First ensure we have complete rows by slicing to multiple of 3 */}
    {displayedStudents
      .slice(0, Math.floor(displayedStudents.length / 3) * 3)
      .map((student) => (
        <div key={student.id} className="bg-white border border-gray-200 rounded-lg shadow-sm p-4 hover:shadow-md transition-shadow text-center flex flex-col h-full">
          {/* Student Image */}
          ...
          {/* Read More Button */}
          <button className="bg-pink-500 hover:bg-pink-600 text-white px-4 py-1.5 rounded-full text-xs font-medium transition-colors mt-2 mx-auto">
            Read More
          </button>
        </div>
      ))
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
              <button
                ...
                className={`px-3 py-1 border rounded-md text-sm ${
                  currentPage === page ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-700 border-gray-300'
                }`}
              >
                {page}
              </button>
